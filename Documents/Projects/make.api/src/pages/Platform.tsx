...
import { Home, LayoutDashboard, Users, Settings, Code, Cloud, Database, Zap, ShoppingCart, MessageSquare, Image, Music, Video, Map, CreditCard, Mail, Lock, BarChart, Globe, Smartphone, Menu, X } from 'lucide-react';
...
export default function APITemplatePlatform() {
  ...
  return (
    <div className="flex h-screen bg-gray-50 ml-6 ">
      {/* Lateral Sidebar */}
      <aside className={`${isSidebarOpen ? 'w-64' : 'w-20'} bg-gradient-to-b from-purple-700 to-purple-900 text-white flex flex-col transition-all duration-300`}>
          ...
          <div className="flex items-center gap-4 mb-6">
            ...
            <div className="flex items-center gap-1 flex-1">
              <div className="w-12 h-12 rounded-full flex items-center justify-center flex-shrink-0">
                <Zap className="text-purple-600 primary" size={32} />
              </div>
              <div>
                <h2 className="text-3xl font-bold text-gray-800" style={{ fontFamily: 'Inter, Helvetica, sans-serif', letterSpacing: '-0.5px' }}>make.api</h2>
                <p className="text-gray-500 mt-0">Pesquise, crie, inicie, teste e deploy</p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6">
            {filteredTemplates.map((template) => {
              ...
              return (
                <div
                  key={template.id}
                  className="bg-white rounded-xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden group cursor-pointer hover:border-purple-300"
                >
                  <div className="p-6">
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-gray-100">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-medium text-gray-700">{template.language}</span>
                      </div>
